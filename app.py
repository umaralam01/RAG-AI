...
def upload():
    ...
    try:
        if file.filename.endswith(".pdf"):
            ...
            db.session.add(UploadLog(filename=file.filename, filetype="pdf"))
            db.session.commit()
        elif file.filename.endswith((".mp3", ".wav")):
            msg = process_audio(file_path)
            db.session.add(UploadLog(filename=file.filename, filetype="audio"))
            db.session.commit()
        elif file.filename.lower().endswith((".png", ".jpg", ".jpeg")):
            extracted_text = pytesseract.image_to_string(Image.open(file_path))
            db.session.add(UploadLog(filename=file.filename, filetype="image"))
            db.session.commit()
            collection.add(
    ...
    return jsonify({"success": True, "message": msg, "filename": file.filename})
...
def ask():
    ...
    if not results["documents"] or not results["documents"][0]:
        return "No relevant content found.", 200, {'Content-Type': 'text/plain'}
    contexts = []
    ...
    def generate_stream():
        try:
            r = requests.post("http://localhost:11434/api/generate",
                              json={"model":"llama3.2","prompt":prompt,"stream":True}, stream=True)
            answer_full = ""
            for line in r.iter_lines():
                if line:
                    ...
                    yield token
            with app.app_context():
                db.session.add(QueryLog(question=query, answer=answer_full))
                db.session.commit()
        except Exception as e:
            yield f"\nStream error: {e}"
